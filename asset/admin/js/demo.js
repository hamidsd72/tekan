...
$(".custom-file-input").on("change", function() {
  ...
  if(fileName)
  {
    $(this).siblings(".custom-file-label").addClass("selected").html('فایل با پسوند "'+type+'" انتخاب شد');
  }
  else
  {
    $(this).siblings(".custom-file-label").addClass("selected").html('انتخاب فایل');
  }
});

function slugify(string) {
  return string.toString().toLowerCase().replace(/\s+/g, '-').replace(/\-\-+/g, '-').replace(/^-+/, '').replace('%', '-درصد').replace(/-+$/, '');
}
...
$('.user_selected_off').on('change',function () {
  if($(this).val()=='0')
  {
    $('.inventory_off').show(200);
  }
  else
  {
    $('.inventory_off').hide(200);
  }
})
